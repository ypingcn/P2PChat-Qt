...
void chatWorker::sendJson(MessageType type, QString nick_name, QString content)
{
    QJsonObject obj;
    ...
    while (messageReader->hasPendingDatagrams())
    {
        ...
        if(jsonError.error == QJsonParseError::NoError && doc.isObject())
        {
            ...
            if(obj.contains("type") && obj.contains("nick-name"))
            {
                ...
                {
                    emit messageShowReady(chatWorker::Chat,info,obj.take("content").toString());
                }
                ...
                else if(type.toString() == "logout")
                {
                    ...
                    emit messageShowReady(chatWorker::Logout,info,tr(" -- quit the chat room"));
                }
